...
const RobloxChatViewer = ({ showToast }) => {
  ...
  // Mobile navigation step: 'users' | 'convs' | 'messages'
  ...
  const handleSelectUser = (u) => {
    ...
    if (isMobile) setMobileStep('convs');
  };
  ...
  const handleSelectConv = (c) => {
    ...
    if (isMobile) setMobileStep('messages');
  };

  const handleMobileBack = () => {
    if (mobileStep === 'messages') {
      setMobileStep('convs');
      setSelectedConv(null);
      setMessages([]);
    } else if (mobileStep === 'convs') {
      setMobileStep('users');
  ...
  const usersPanel = (
    <div style={{
      ...panelBase,
      ...(isMobile
        ? { display: mobileStep === 'users' ? 'flex' : 'none', flex: 1, padding: '1rem' }
        : { flex: '0 0 220px', padding: '1rem', overflowY: 'auto' })
    }}>
      <h3 style={{ margin: '0 0 1rem 0', fontSize: '0.95rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.4rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' }}>
        👥 Pengguna Ber-Cookie
      </h3>
      {isLoadingUsers ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', overflowY: 'auto', flex: 1 }}>
          {users.map((u) => {
            ...
            return (
              ...
              >
                {u.avatar_url ? (
                  <img src={u.avatar_url} alt="" style={{ width: 36, height: 36, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.1)', flexShrink: 0 }} />
                ) : (
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#475569', flexShrink: 0 }} />
                )}
                <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                  <span style={{ fontWeight: 600, fontSize: '0.85rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: '#fff' }}>
                ...
                {isMobile && (
                  <span style={{ marginLeft: 'auto', color: '#475569', fontSize: '1rem', flexShrink: 0 }}>›</span>
                )}
              </div>
            );
          })}
  ...
  const convsPanel = (
    <div style={{
      ...panelBase,
      ...(isMobile
        ? { display: mobileStep === 'convs' ? 'flex' : 'none', flex: 1, padding: '1rem' }
        : { flex: '0 0 270px', padding: '1rem', display: 'flex' })
    }}>
      {isMobile && (
        <button style={backBtnStyle} onClick={handleMobileBack}>
          ‹ Pengguna
        </button>
      )}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '0 0 0.75rem 0', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem', gap: '0.5rem' }}>
        <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.4rem', flexShrink: 0 }}>
          💬 {isMobile && selectedUser ? (selectedUser.roblox_display_name || selectedUser.roblox_username) : 'Daftar Obrolan'}
        </h3>
        {selectedUser && (
          <button
      ...
      {selectedUser ? (
              ...
              filteredConversations.map((c, index) => {
                ...
                return (
                  <div
                    ...
                    onMouseOut={(e) => { if (!isSelected) e.currentTarget.style.background = 'rgba(255,255,255,0.01)'; }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', flexShrink: 0 }}>
                        ...
                        {isMobile && <span style={{ color: '#475569', fontSize: '1rem' }}>›</span>}
                      </div>
                    </div>
                    <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)', textTransform: 'capitalize' }}>
                      {isGroup ? 'Grup Party' : 'Pesan Langsung'}
                    </span>
                  </div>
                );
              })
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '0.85rem', textAlign: 'center', padding: '1rem' }}>
          Pilih pengguna untuk melihat daftar obrolan.
        </div>
      )}
    </div>
  );
  ...
  const messagesPanel = (
    <div style={{
      ...
      overflow: 'hidden',
      ...(isMobile
        ? { display: mobileStep === 'messages' ? 'flex' : 'none', flex: 1 }
        : { flex: 1 })
    }}>
      {selectedConv ? (
        <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', minWidth: 0 }}>
              {isMobile && (
                <button
                  ...
                  style={{ background: 'none', border: 'none', color: '#60a5fa', fontSize: '1.2rem', cursor: 'pointer', padding: '0 0.25rem', flexShrink: 0 }}
                >
                  ‹
                </button>
              )}
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {selectedConv.id || 'Lokal (Tanpa ID)'}
                </div>
              </div>
            </div>
            {selectedConv.id && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '0.5rem', color: 'var(--text-muted)', textAlign: 'center', padding: '2rem' }}>
          ...
          <span style={{ fontSize: '0.85rem' }}>Pilih obrolan untuk menampilkan riwayat pesan.</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{
      display: 'flex',
      gap: '0.75rem',
      height: isMobile ? 'calc(100vh - 180px)' : 'calc(100vh - 240px)',
